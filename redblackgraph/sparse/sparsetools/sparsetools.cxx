...
call_thunk(char ret_spec, const char *spec, thunk_t *thunk, PyObject *args)
{
    ...
    for (p = spec; *p != '\0'; ++p, ++j) {
        ...
        if (*p == '*') {
            ...
            /* Cast if necessary */
            arg = arg_arrays[j];
            if (PyArray_EquivTypenums(PyArray_DESCR((PyArrayObject*)arg)->type_num, cur_typenum)) {
                /* No cast needed. */
            }
            else if (!is_output[j] || PyArray_CanCastSafely(cur_typenum, PyArray_DESCR((PyArrayObject*)arg)->type_num)) {
...
static PyObject *c_array_from_object(PyObject *obj, int typenum, int is_output)
{
    if (!is_output) {
        ...
        #else
            int flags = NPY_ARRAY_C_CONTIGUOUS|NPY_ARRAY_WRITEABLE|NPY_ARRAY_UPDATEIFCOPY|NPY_ARRAY_NOTSWAPPED;
        #endif
        if (typenum == -1) {
...
PyMODINIT_FUNC PyInit__sparsetools(void)
{
    PyObject *m;
