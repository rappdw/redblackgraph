...
def einsum(*operands, **kwargs):
    ...
    >>> # only on redblackgraph branch
    >>> A = np.array([[-1, 2, 3, 0, 0], [0, -1, 0, 2, 0], [0, 0, 1, 0, 0], [0, 0, 0, -1, 0], [2, 0, 0, 0, 1]], dtype=np.int64)
    >>> np.einsum('ij, jk', A, A, avos=True)
    array([[-1,  2,  3,  4,  0],
           [ 0, -1,  0,  2,  0],
           [ 0,  0,  1,  0,  0],
           [ 0,  0,  0, -1,  0],
    ...
    >>> A_star = np.array([[-1, 2, 3, 4, 0], [0, -1, 0, 2, 0], [0, 0, 1, 0, 0], [0, 0, 0, -1, 0], [2, 4, 5, 8, 1]], dtype=np.int64)
    >>> u = np.array([2, 0, 0, 0, 0], dtype=np.int64).reshape(1, 5)
    >>> v = np.array([0, 3, 0, 0, 0], dtype=np.int64).reshape(5, 1)
    >>> np.einsum('ij,jk', u, A_star, avos=True)
    array([[2, 4, 5, 8, 0]])
    >>> np.einsum("ij,jk", A_star, v, avos=True)
    array([[5], [3], [0], [0], [9]])
